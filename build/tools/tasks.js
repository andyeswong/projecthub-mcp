...
export function registerTaskTools(server) {
    // ── List ──────────────────────────────────────────────────────────────
    server.tool('task_list', `List tasks in a project. Supports filtering by status, assignee, and priority.
...
Use assignee="me" to get tasks assigned to the current agent.`, {
    ...
    // ── Create ────────────────────────────────────────────────────────────
    server.tool('task_create', `Create a single task in a project.
...
Returns the full task object with ID for future updates.`, {
    ...
    // ── Batch create ──────────────────────────────────────────────────────
    server.tool('task_batch', `Create up to 50 tasks in a single request. Much faster than calling task_create in a loop.
...
Ideal for decomposing a project into its initial task set.`, {
    ...
    // ── Get ───────────────────────────────────────────────────────────────
    server.tool('task_get', 'Get full task detail including comments and activity timeline.', {
    ...
    // ── Update ────────────────────────────────────────────────────────────
    server.tool('task_update', `Update task fields. All fields are optional — only send what changes.
...
Status changes are automatically recorded in the event log.`, {
    ...
    // ── Archive ───────────────────────────────────────────────────────────
    server.tool('task_archive', `Soft-delete a task. Archived tasks are hidden from task lists by default.
...
Use task_unarchive to restore it.`, {
    ...
    // ── Unarchive ─────────────────────────────────────────────────────────
    server.tool('task_unarchive', 'Restore an archived task. Clears archived_at, archived_by, and archive_reason. Emits task.unarchived.', {
        id: z.string().uuid().describe('Task UUID'),
    }, async ({ id }) => {
