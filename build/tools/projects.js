...
export function registerProjectTools(server) {
    // ── List ──────────────────────────────────────────────────────────────
    server.tool('project_list', `List projects in the current organization. Scoped to the agent's org automatically.
Use workspace filter to narrow by workspace slug.`, {
    ...
    // ── Get ───────────────────────────────────────────────────────────────
    server.tool('project_get', 'Get a single project with task counts by status.', {
    ...
    // ── Create ────────────────────────────────────────────────────────────
    server.tool('project_create', 'Create a new project inside a workspace.', {
    ...
    // ── Update ────────────────────────────────────────────────────────────
    server.tool('project_update', 'Update project name, description, or status.', {
        id: z.string().uuid().describe('Project UUID'),
    ...
    // ── Workspaces ────────────────────────────────────────────────────────
    server.tool('workspace_list', 'List workspaces in an organization.', {
        org_slug: z.string().describe('Organization slug'),
    }, async ({ org_slug }) => {
        const result = await api.get(`/organizations/${org_slug}/workspaces`);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    });
    server.tool('workspace_create', 'Create a new workspace inside an organization.', {
        org_slug: z.string().describe('Organization slug'),
